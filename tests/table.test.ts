...
describe("AirtableTable", function () {
  ...
  test("PATCH update multiple records - /:baseId/:tableId", async function () {
    ...
    expect(response.records[0].get("Notes")).toEqual(
      "Real Estate Investment Trusts"
    );
    expect(response.records[0].createdTime).toBeInstanceOf(Date);
  });

  test("PATCH upsert multiple records - /:baseId/:tableId", async function () {
    mockResponses(200, {
      records: [
        {
          id: "recPaibwSLDbZr80V",
        ...
        {
          id: "recgKHlQ81Lq8Qx0r",
          createdTime: "2024-12-22T13:07:10.000Z",
          fields: { Name: "Upserted Record" },
        },
      ],
      updatedRecords: ["recPaibwSLDbZr80V"],
      createdRecords: ["recgKHlQ81Lq8Qx0r"],
    });
    ...
    const response = await table.update(
      "PATCH",
      [
        { id: "recPaibwSLDbZr80V", fields: { Name: "Updated Record" } },
        { fields: { Name: "Upserted Record" } },
      ],
    ...
    expect(response.records[0]).toBeInstanceOf(AirtableRecord);
    expect(response.records[0].id).toEqual("recPaibwSLDbZr80V");
    expect(response.records[0].get("Name")).toEqual("Updated Record");
    expect(response.records[0].get("Notes")).toEqual(
      "Real Estate Investment Trusts"
    );
    expect(response.records[0].createdTime).toBeInstanceOf(Date);

    expect(response.records[1]).toBeInstanceOf(AirtableRecord);
    expect(response.records[1].id).toEqual("recgKHlQ81Lq8Qx0r");
    expect(response.records[1].get("Name")).toEqual("Upserted Record");
    expect(response.records[1].createdTime).toBeInstanceOf(Date);

    expect(response.updatedRecords).toContain("recPaibwSLDbZr80V");
    expect(response.createdRecords).toContain("recgKHlQ81Lq8Qx0r");
  });
  ...
  test("PUT upsert multiple records - /:baseId/:tableId", async function () {
    mockResponses(200, {
      records: [
        {
          id: "recPaibwSLDbZr80V",
  ...
  test("Delete records - /:baseId/:tableId", async function () {
    mockResponses(200, {
      records: [
        { id: "recWlZekWgCmWkPJb", deleted: true },
        { id: "recfEBIWpwnYYI9WH", deleted: true },
      ],
    });
    ...
    expect(response.records[0]).toMatchObject({
      id: "recWlZekWgCmWkPJb",
      deleted: true,
    });
    expect(response.records[1]).toMatchObject({
      id: "recfEBIWpwnYYI9WH",
      deleted: true,
    });
  });

  // Handy interface from base()
  test("Upload attachment - /:baseId/:recordId/:attachmentFieldIdOrName/uploadAttachment", async function () {
    mockResponses(200, {
      id: "recPaibwSLDbZr80V",
      createdTime: "2024-12-15T04:20:33.000Z",
      fields: {
    ...
    const response = await airtable
      .base("appEpvhkjHcG8OvKu")
      .table("tblc7ieWKVQM9eequ")
      .uploadAttachment("recPaibwSLDbZr80V", "fldw7KUUaL5nVAKNU", {
        filename: "new_attachment.png",
  ...
  test("Sync CSV - /:baseId/:tableId/sync/:apiEndpointSyncId", async function () {
    mockResponses(200, { success: true });

    const table = new Airtable()
      .base("appEpvhkjHcG8OvKu")
      .table("tblHxzW6ZA0BOFW90");

    const response = await table.syncCSV(
      "5Frr87Ft",
      // Data snippet from {@link https://github.com/datablist/sample-csv-files?}
